...
func TestBatchCompilesMatchOneShotCompiles(t *testing.T) {
	compiler, packs, directory := batchTestEnvironment(t)
...
func TestBatchCompilerSurvivesAProgramItCannotCompile(t *testing.T) {
	...
	responses := runBatchCompiler(t, compiler, packs, []batchRequest{
...
func TestBatchCompilerSharesItsWorldAcrossPrograms(t *testing.T) {
	...
	responses := runBatchCompiler(t, compiler, packs, requests)
...
// to a file the command line can name, and returns a scratch directory. packs is
// goc's -runtime value, which is a comma-separated set; here it names one.
func batchTestEnvironment(t *testing.T) (compiler, packs, directory string) {
	t.Helper()
	...
	directory = t.TempDir()
	packPath := filepath.Join(directory, "runtime.gocrt")
	require.NoError(t, sharedPrebuiltRuntime(t).Write(packPath))
	return sharedGOCBinary(t), packPath, directory
}
...
func compileEachAloneForTest(t *testing.T, compiler, packs, directory, suffix string, sources []string) map[string][]byte {
	t.Helper()
	...
	for _, source := range sources {
		...
		go func(source string) {
			...
			combined, err := exec.Command(compiler, "-runtime", packs, "-o", output, source).CombinedOutput()
			if err != nil {
				mutex.Lock()
...
func compileBatchForTest(t *testing.T, compiler, packs, directory, suffix string, sources []string) map[string][]byte {
	t.Helper()
...
func runBatchCompiler(t *testing.T, compiler, packs string, requests []batchRequest) []batchResponse {
	t.Helper()

	command := exec.Command(compiler, "compile-batch", "-runtime", packs)
