...
//
// What is shared is goc's source world -- the parsed and type-checked closure of
...
//
// The prebuilt runtimes are shared too, but not in the same way. -runtime takes a
// set, and which pack a program gets depends on what that program loaded, so the
// set cannot be collapsed to one pack at startup. What a worker holds instead is
// every candidate's manifest -- which is all the choice needs -- and each pack's
// objects from the first program in that worker that picks it. See packSet.
//
...
//
// Target, prebuilt runtimes and -O select which files a package is built from and
...
func compileBatchCommand(arguments []string, input io.Reader, output io.Writer, errorOutput io.Writer) int {
	...
	for requests.Scan() {
		...
		response := compileBatchRequest(target, packs, *optimize, request)
...
func compileBatchRequest(target goc.Target, packs *packSet, optimize bool, request batchRequest) (response batchResponse) {
	started := time.Now()
	...
	if err := compileBatchProgram(target, packs, optimize, request.Source, request.Output); err != nil {
		response.Error = err.Error()
	}
	return response
}

// compileBatchProgram is the batch equivalent of one `goc [-runtime packs] -o out
...
func compileBatchProgram(target goc.Target, packs *packSet, optimize bool, source, output string) error {
