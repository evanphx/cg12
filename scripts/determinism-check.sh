...
#
# Default: the five sample programs, each built cold (CG12_NOCACHE=1) and warm,
# twice, so a disagreement between the two caching paths shows up as well as one
# between two runs of the same path. All five must match; the four hashes printed
# for a program must all be equal, across rounds as well as within one.
#
...
for program in "${programs[@]}"; do
	...
	for round in 1 2; do
		...
		CG12_NOCACHE=1 "$work/goc" "${extra[@]+"${extra[@]}"}" -o "$cold" "$source" >/dev/null 2>&1 || { verdict="$verdict round$round:COLD-BUILD-FAILED "; continue; }
		"$work/goc" "${extra[@]+"${extra[@]}"}" -o "$warm" "$source" >/dev/null 2>&1 || { verdict="$verdict round$round:WARM-BUILD-FAILED "; continue; }
