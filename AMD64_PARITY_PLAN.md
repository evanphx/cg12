...
| --- | --- | --- |
| A1 | **Atomics** — all 22 intrinsics. `grep -c atomic amd64/*.go` = 0 today. Nothing in Go's runtime works without CAS. *Simpler* than arm64: single-instruction, no LL/SC retry loop. RAX already reserved. | `x64/atomic.go`, `amd64/xasm_atomic.go`, `amd64/xselect_atomic.go` |
| A2 | **Bit ops** — `OClz` (`BSR`/`LZCNT`), `OUMulh`/`OSMulh` (one-operand `MUL`/`IMUL`), `POPCNT`, `ROL`/`ROR`, `SHLD`/`SHRD`, `BSWAP` | `x64/bits.go`, `amd64/xasm_bits.go`, `amd64/xselect_bits.go` |
| A3 | **Immediates + CMOV** — `x64.AddImm`/`AndImm`/`OrImm`/`XorImm`/`ImulImm`/`Cmovcc` all exist and are used **zero** times. Biggest code-quality win. Mirror `arm64/imm.go`. | `amd64/imm.go`, `amd64/xasm_imm.go`, `amd64/xselect_imm.go` |
| A4 | **128-bit memory** — `OLoadq`/`OStoreq`, entirely unhandled. `MOVDQU`/`MOVDQA`/`MOVUPS`. Remove the skip at `foldaddr.go:41-45`. | `x64/sse2.go`, `amd64/xasm_wide.go`, `amd64/xselect_wide.go` |
| A5 | **TLS models** — local-exec is hardcoded; add `Options.TLSModel` mirroring `arm64/mc.go:36-56`, plus GOTTPOFF/TLSGD | `amd64/tls.go`, `x64/tls.go`, `amd64/xasm_tls.go`, `amd64/xselect_tls.go` |
