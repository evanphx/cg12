...
func isComplex(t cc.Type) bool {
	switch t.Kind() {
	case cc.ComplexFloat, cc.ComplexDouble:
		return true
	}
	return false
}
...
func complexCls(t cc.Type) ir.Cls {
	if t.Kind() == cc.ComplexFloat {
		return ir.ClsS
	}
	return ir.ClsD
}
...
func complexSub(t cc.Type) ir.SubCls {
	if t.Kind() == cc.ComplexFloat {
		return ir.SubS
	}
	return ir.SubD
...
func complexCompSize(t cc.Type) int {
	if t.Kind() == cc.ComplexFloat {
		return 4
	}
	return 8
...
func (g *gen) complexAgg(t cc.Type) *ir.AggType {
	sub := complexSub(t)
...
func (g *gen) toComponent(v ir.Ref, from cc.Type, comp ir.Cls) ir.Ref {
...
func (g *gen) complexParts(e cc.ExpressionNode, comp ir.Cls) (re, im ir.Ref) {
...
func (g *gen) complexArithComp(ln, rn cc.ExpressionNode, resT cc.Type) (ir.Cls, bool) {
	if isComplex(resT) {
...
func (g *gen) complexArith(op string, ln, rn cc.ExpressionNode, comp ir.Cls) ir.Ref {
...
func (g *gen) complexCompound(op string, addr ir.Ref, t cc.Type, rhs cc.ExpressionNode) ir.Ref {
	comp := complexCls(t)
...
func (g *gen) complexNeg(e cc.ExpressionNode, comp ir.Cls) ir.Ref {
...
func (g *gen) complexCompare(op string, ln, rn cc.ExpressionNode) ir.Ref {
	comp := ir.ClsS
...
func (g *gen) effComplex(e cc.ExpressionNode) (ir.Cls, bool) {
	if e == nil {
		return 0, false
	}
	if isComplex(e.Type()) {
	...
	case *cc.MultiplicativeExpression:
		if n.Case == cc.MultiplicativeExpressionMul || n.Case == cc.MultiplicativeExpressionDiv {
	...
	case *cc.UnaryExpression:
		if n.Case == cc.UnaryExpressionMinus || n.Case == cc.UnaryExpressionPlus {
...
func (g *gen) effComplexBin(l, r cc.ExpressionNode) (ir.Cls, bool) {
...
// do not).
func compRankDouble(e cc.ExpressionNode) bool {
	switch e.Type().Kind() {
	case cc.Double, cc.LongDouble, cc.ComplexDouble, cc.ComplexLongDouble:
		return true
	}
	return false
}
...
func complexConstVal(e cc.ExpressionNode) (complex128, bool) {
...
// constants.
func constComplex(e cc.ExpressionNode) (complex128, bool) {
	if e == nil {
		return 0, false
	}
	...
	if fv, ok := e.Value().(cc.Float64Value); ok {
...
func (g *gen) complexConvert(v ir.Ref, from, to cc.Type) ir.Ref {
	switch {
...
func (g *gen) complexToComplex(v ir.Ref, src ir.Cls, to cc.Type) ir.Ref {
	comp := complexCls(to)
	if src == comp {
		return v
	}
...
func (g *gen) complexToReal(v ir.Ref, src ir.Cls, to cc.Type) ir.Ref {
	re := g.cur.Load(src, v)
	if to.Kind() == cc.Bool {
...
func complexOperand(n *cc.UnaryExpression) cc.ExpressionNode {
...
func (g *gen) complexReal(e cc.ExpressionNode) ir.Ref {
	if isComplex(e.Type()) {
...
func (g *gen) complexImag(e cc.ExpressionNode) ir.Ref {
	if isComplex(e.Type()) {
		addr := g.genExpr(e)
