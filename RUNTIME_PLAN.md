...
- **`internal/poll/writev.go`'s `&chunk[0]`** was treated as addressing the slice
  header, so `chunk` was heap-lifted. §25 narrows `findEscapingCaptures` exactly
...
- **`runtime.printfloat32`, `printfloat64`, `printcomplex64` and
  `printcomplex128` heap-allocated their scratch array.** Closed by §25, which
...
superseded: §25 closes the same defect ungated, together with the `&v.a.b` and
...
## 25. A parameter can leak only to a result, and a field address is a use (2026-08-01)
